...
load("/home/runner/projects/c2000-ota-update/test/debug-server-scripting/json_support.js");

/* This Object Oriented Javascript code. */
/* We define an object type DSSServer that will handle access to all the sessions of DebugServer.

   var server = new DSSServer("config_file.ccxml", 4444, "logfile.xml", 50000);
   server.run();
   server.shutdown();
...
*/
function DSSServer(board_configuration, base_port) {
    ...
    /* Predefined handlers supported out-of-the-box on the DSSServer sessions.
       More can be added very easily, using the addHandlers method of the DSSServer. */
    this.handlers = {
...
DSSServer.prototype.addHandlers = function(handlers) {
...
DSSServer.prototype.run = function () {
    for(var i = 0; i < this.ports.length; i++) {
        this.threads[i].start();
    }
    print("DSSServer is ready.");
...
DSSServer.prototype.shutdown = function () {
    ...
    print("DSSServer is stopped.");
}

DSSServer.prototype.createSessionThread = function (socket, session) {
    var server = this;                  
    return new Thread(
        new Runnable() {
          run: function() {
...
/* Predefined handlers supported out-of-the-box on the DSSServer. */
